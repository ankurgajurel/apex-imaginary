...
import { BLOG } from "@/lib/api/blogs/getBlogBySlug";
import { useQuery } from "@apollo/client";

import ParsedMarkdown from "../Universal/ParsedMarkdown/ParsedMarkdown";

export default function BlogComponent({ slug }: { slug: string }) {
  ...
  function getArrayFromCommaSeparatedWords(inputString: string): string[] {
  ...
  function calculateReadTime(
    characters: number,
    charactersPerMinute: number = 200
  ): string {
    ...
    if (roundedMinutes === 1) {
      ...
      return `${roundedMinutes} minutes read`;
    }
  }

  const currentBlog = data?.blogs?.data[0]?.attributes;
  return (
    <div>
      <div className="max-w-3xl px-4 pt-6 lg:pt-10 pb-12 sm:px-6 lg:px-8 mx-auto">
        <div className="max-w-2xl">
                ...
                <div className="flex justify-between items-center gap-x-2">
                  <div>
                    ...
                    <ul className="text-xs text-gray-500">
                      ...
                      <li className="inline-block relative pe-6 last:pe-0 last-of-type:before:hidden before:absolute before:top-1/2 before:end-2 before:-translate-y-1/2 before:size-1 before:bg-gray-300 before:rounded-full dark:text-gray-400 dark:before:bg-gray-600">
                        {calculateReadTime(currentBlog?.content.length)}
                      </li>
                    </ul>
                  </div>
          ...
          <figure className="w-full">
            <img
              className="w-fit mx-auto object-contain min-h-[50vh] rounded-2xl"
              src={
                process.env.NEXT_PUBLIC_ASSET_URL +
                currentBlog?.cover.data.attributes.url
              }
              alt="Image Description"
            />
          </figure>

          <div className="space-y-5 md:space-y-8 mt-10">
            ...
            <ParsedMarkdown>{currentBlog?.content}</ParsedMarkdown>

            <div>
              {getArrayFromCommaSeparatedWords(currentBlog?.tags)?.map(
                (tag: string, index: number) => (
                  <span className="m-1 inline-flex items-center gap-1.5 py-2 px-3 rounded-full text-sm bg-gray-100 text-gray-800 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-200 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                    {tag}
                  </span>
                )
              )}
