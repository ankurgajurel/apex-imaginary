...
import { TEAMS } from "@/lib/api/team/getAllTeams";
import { useQuery } from "@apollo/client";

export default function TeamComponent() {
  const { data } = useQuery(TEAMS);
  const teamData = data?.teams?.data;
  return (
    <>
      <div>
        <div>
          <div>
            <div className="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {teamData?.map((teamMember: any, index: number) => (
                  <div key={index} className="flex flex-col rounded-xl p-4 md:p-6 bg-white border border-gray-200 dark:bg-slate-900 dark:border-gray-700">
                    <div className="flex items-center gap-x-4">
                      <img
                        className="rounded-full size-20"
                        src={teamMember.attributes.profileGithub}
                      ...
                      <div className="grow">
                        ...
                        <p className="text-xs uppercase text-gray-500">
                          {teamMember.attributes.designation}
                        </p>
                      </div>
                    </div>

                    <p className="mt-3 text-gray-500">
                      {teamMember.attributes.headline}
                    </p>
                    ...
                    <div className="mt-3 space-x-1">
                      <a
                        ...
                        href={teamMember.attributes.github}
                      >
                        <svg
